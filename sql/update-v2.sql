-- ============================================================
-- LOCK-IN — SQL UPDATE V2 (Tables exist, fix RLS + conflicts)
-- Run this in Supabase SQL Editor after initial schema
-- ============================================================

-- 1. Ensure all tables exist (safe if already created)
...
CREATE TABLE IF NOT EXISTS time_blocks (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(user_id, day, start_time, label)
);
...
-- 2. Leaderboard view (recreate safely)
...
-- 3. Enable RLS on every member-facing table
...
-- 4. RLS Policies (recreate/drop and recreate)
-- Profiles
...
CREATE POLICY "Profiles: select all members" ON profiles FOR SELECT USING (true);
...
CREATE POLICY "Cohorts: admin manage" ON cohorts FOR ALL USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

-- Device Sessions
DROP POLICY IF EXISTS "Device: user only" ON device_sessions;
CREATE POLICY "Device: user only" ON device_sessions FOR ALL USING (auth.uid() = user_id);

-- Time Blocks
DROP POLICY IF EXISTS "Time blocks: user only" ON time_blocks;
CREATE POLICY "Time blocks: user only" ON time_blocks FOR ALL USING (auth.uid() = user_id);

-- Check-ins
DROP POLICY IF EXISTS "Check-ins: user only" ON check_ins;
CREATE POLICY "Check-ins: user only" ON check_ins FOR ALL USING (auth.uid() = user_id);

-- Streaks
DROP POLICY IF EXISTS "Streaks: user only" ON streaks;
CREATE POLICY "Streaks: user only" ON streaks FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Team members: user sees own" ON team_members FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Team members: user manages own" ON team_members FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Team members: user deletes own" ON team_members FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Team log: team members read" ON team_startup_log FOR SELECT USING (EXISTS (SELECT 1 FROM team_members WHERE team_id = team_startup_log.team_id AND user_id = auth.uid()));
CREATE POLICY "Team log: team members insert" ON team_startup_log FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM team_members WHERE team_id = team_startup_log.team_id AND user_id = auth.uid()));
...
CREATE POLICY "Reports: admin manage" ON reports FOR ALL USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
...
CREATE POLICY "Community: admin manage" ON community_posts FOR ALL USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

-- Reminders
DROP POLICY IF EXISTS "Reminders: user only" ON reminders;
CREATE POLICY "Reminders: user only" ON reminders FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Teams: admin manage" ON teams FOR ALL USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

-- 5. Trigger / Function (safe recreation)
...
DROP TRIGGER IF EXISTS trigger_update_streak ON check_ins;
CREATE TRIGGER trigger_update_streak
AFTER INSERT OR UPDATE ON check_ins
FOR EACH ROW EXECUTE FUNCTION update_streak();

-- 6. Seed data (fixed — uses conflict target on time_blocks unique constraint)
-- Only insert if profiles exists
DO $$
DECLARE
  profile_id UUID;
BEGIN
  ...
  IF profile_id IS NOT NULL THEN
    ...
    VALUES
      (profile_id, 1, '06:00', '09:00', 'Deep Work Block 1', 'work'),
      (profile_id, 1, '09:00', '09:30', 'Protected Break', 'break'),
      (profile_id, 1, '09:30', '12:00', 'Deep Work Block 2', 'work'),
      (profile_id, 1, '12:00', '12:30', 'Movement', 'movement'),
      (profile_id, 1, '12:30', '13:00', 'Reflection / Journal', 'reflection'),
      (profile_id, 1, '13:00', '16:00', 'Deep Work Block 3', 'work')
    ON CONFLICT (user_id, day, start_time, label) DO NOTHING;
  END IF;
END $$;
